...
namespace Puchase_and_payables.Controllers.V1
{
    ...
    public class OnWebSupplierController : Controller
    {
        ...
        [HttpGet(ApiRoutes.OnWebSupplierEndpoints.GET_SUGGESTED_SUPPLIER_BIDs)]
        public async Task<ActionResult> GET_SUGGESTED_SUPPLIER_BIDs()
        {
            var query = new GetSuggestedSupplierAdvertsQuery();
            return Ok(await _mediator.Send(query));
        }


        [HttpGet(ApiRoutes.OnWebSupplierEndpoints.GET_DASHBOARD_COUNT)]
        public async Task<ActionResult> GET_DASHBOARD_COUNT([FromQuery] SupplierDashboardCountQuery query)
        { 
            return Ok(await _mediator.Send(query));
        }  
    }
