...
using Puchase_and_payables.Handlers.Purchase.PRNs;
using Puchase_and_payables.Handlers.Requirement;
using System.Threading.Tasks;
using static Puchase_and_payables.Handlers.Requirement.ERPActivityAttribute;
...
namespace Puchase_and_payables.Controllers.V1
{
    ...
    public class PurchaseController : Controller
    {
        ...
        [AllowAnonymous]
        [HttpPost(ApiRoutes.Purchases.MODIFY_PRN_DETAILS)]
        public async Task<ActionResult> MODIFY_PRN_DETAILS([FromBody] ModifyPRNDetailCommand command)
        {
            var resp = await _mediator.Send(command);
            if (!resp.Status.IsSuccessful)
                return BadRequest(resp);
        ...
        [AllowAnonymous]
        [HttpPost(ApiRoutes.Purchases.MODIFY_BIDS)]
        public async Task<ActionResult> MODIFY_BIDS([FromBody] ModifyBidCommand command)
        {
            var resp = await _mediator.Send(command);
            if (!resp.Status.IsSuccessful)
                return BadRequest(resp);
