    ...
    public class AddUpdateBidAndTenderBySupplierCommandHandler : IRequestHandler<AddUpdateBidAndTenderCommand, BidAndTenderRegRespObj>
    {
        ...
        public async Task<BidAndTenderRegRespObj> Handle(AddUpdateBidAndTenderCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var bidAndTenderObj = await BuildBidAndTenderObject(request); 
        ...
        private async Task<cor_bid_and_tender> BuildBidAndTenderObject(AddUpdateBidAndTenderCommand request)
        {
            ...
            var bid = await _dataContext.cor_bid_and_tender.FirstOrDefaultAsync(w => w.SupplierId == request.SupplierId && w.LPOnumber.Trim().ToLower() == request.LPONumber.Trim().ToLower());
            if (bid == null)
            {
            ...
            {
                bid.BidAndTenderId = bid.BidAndTenderId;
            }
