    ...
    public class GetBidAndTenderAwaitingApprovalQueryHandler : IRequestHandler<GetBidAndTenderAwaitingApprovalQuery, BidAndTenderRespObj>
    {
        ...
        public async Task<BidAndTenderRespObj> Handle(GetBidAndTenderAwaitingApprovalQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var target_ids = res.workflowTasks.Select(x => x.TargetId).ToList();
                var workflow_tokens = res.workflowTasks.Select(s => s.WorkflowToken).ToList();
                var staffBidawaiting = await _repo.GetBidAndTenderAwaitingApprovalAsync(target_ids, workflow_tokens);
                  

                response.BidAndTenders = staffBidawaiting?.Where(w => w.ApprovalStatusId != (int)ApprovalStatus.Disapproved 
                && user.Staff_limit >= w.AmountApproved)
                     .OrderBy(r => r.ApprovalStatusId == (int)ApprovalStatus.Processing)
                    .Select(d => new BidAndTenderObj
                {
                    ...
                    PaymentTerms = _dataContext.cor_paymentterms.Where(s => s.BidAndTenderId == d.BidAndTenderId && (bool)!d.Deleted).Select(a => new PaymentTermsObj
                    {
