            ...
            public async Task<PaymentTermsRegRespObj> Handle(UpdatePaymentProposalCommand request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    using (var tran = await _dataContext.Database.BeginTransactionAsync())
                    {
                        try
                        {
                            var phase = await _dataContext.cor_paymentterms.FirstOrDefaultAsync(r => r.PaymentTermId == request.PaymentTermId);

                            if (phase == null)
                            {
                                response.Status.Message.FriendlyMessage = "Phase not found";
                                return response;
                            }

                            if (phase.Status == (int)JobProgressStatus.Executed_Successfully && phase.PaymentStatus != 0)
                            {
                            ...
                            if (file.Count() > 0)
                            {
                                ...
                                var fileName = $"phase{request.PaymentTermId}-{DateTime.Now.ToString().Split(" ")[1].Replace(':', '-')}";
                                ...
                                phase.CcompletionCertificateName = "/Resources/" + fileName;
                            ...
                            if (phase.Status == (int)JobProgressStatus.Executed_Successfully)
                            {
                                ...
                                {
                                    thisPhaseLPO.CompanyId = thisPhaseLPO.CompanyId == 0 ? _dataContext.purch_requisitionnote.FirstOrDefault(e => e.PurchaseReqNoteId == thisBidLpo.PurchaseReqNoteId).DepartmentId : thisPhaseLPO.CompanyId;
                                    ...
                                    if(entryRequest1 == null)
                                    {
                                        await tran.RollbackAsync();
                                        response.Status.IsSuccessful = false;
                                        response.Status.Message.FriendlyMessage = $"Proposal entry Failed : Process rolled back:  <br/> Supplier type not found";
                                        return response;
                                    }
                                    ...
                                    if (!phaseEntry.Status.IsSuccessful)
                                    {
                                        await tran.RollbackAsync();
                                        response.Status.IsSuccessful = false;
                                        response.Status.Message.FriendlyMessage = $"Proposal entry Failed : Process rolled back:  <br/>{phaseEntry.Status.Message.FriendlyMessage}";
                                        return response;
                                    } 
                                    invoice.CreditGl = entryRequest1.CreditGL;
                                    invoice.DebitGl = entryRequest1.DebitGL;
                                    thisPhaseLPO.DateCompleted = DateTime.UtcNow;
                                    phase.CompletionDate = DateTime.UtcNow;
                                    await _repo.AddUpdatePaymentTermsAsync(phase);
                                    ...
                                    response.Status.Message.FriendlyMessage = "Invoice generated";
                                    return response;
                                }
                            }
                            await tran.CommitAsync();
