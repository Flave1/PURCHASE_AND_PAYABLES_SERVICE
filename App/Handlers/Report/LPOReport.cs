        ...
        public class LPOReportQueryHandler : IRequestHandler<LPOReportQuery, LPOReport>
        {
            ...
            public async Task<LPOReport> Handle(LPOReportQuery request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    var phases = _dataContext.cor_paymentterms.Where(w => w.LPOId == lpo.PLPOId && w.ProposedBy == (int)Proposer.STAFF && w.BidAndTenderId == lpo.BidAndTenderId).Select(w => new Phases
                    {
                        Amount = w.Amount, 
                        Tax = w.TaxPercent,
                        Phase = w.Phase
                    ...
                    response.GrossAmount = lpo.AmountPayable;
