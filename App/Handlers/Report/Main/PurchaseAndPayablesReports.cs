        ...
        public async Task<PurchAndPayaReportResp> Handle(PurchaseAndPayablesReportQuery request, CancellationToken cancellationToken)
        {
            ...
            { 
                ...
                var result = (from a in _dataContext.purch_plpo 
                              ...
                              where a.CreatedOn >= request.FromDate && a.CreatedOn <= request.ToDate
                              select new PurchAndPayaReport
                              {
                                  ...
                                  RequestProcessPeriod = (int)(a.DeliveryDate - a.RequestDate).TotalDays,
                                  ActualDeleiverDate = c.DeliveryDate, 
                              }).ToList();
                foreach (var item in result)
                {
                    item.DepartmentName = comp.companyStructures.FirstOrDefault(w => w.CompanyStructureId == item.DepartmentId)?.Name;
                }
                ...
                response.PurchAndsPayables = result;
                return response;
            }
            catch (Exception ex)
            {
