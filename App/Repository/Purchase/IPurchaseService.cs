...
namespace Puchase_and_payables.Repository.Purchase
{
    public interface IPurchaseService
    {
        ...
        purch_plpo BuildLPODomianObject(purch_prndetails model, purch_requisitionnote prn);
        ...
        Task<bool> LinkThisProposalsToCurrentLPOAsync(cor_bid_and_tender currentBid);
        Task<bool> DisapproveOtherLostbids(cor_bid_and_tender currentBid);
        Task<bool> ApproveCurrentBidForThisLPO(cor_bid_and_tender currentBid, purch_plpo _ThisBidLPO);
        Task<bool> RejectThisBid(cor_bid_and_tender rejectedLpoBid);
        Task<bool> ReactivateOtherBids(cor_bid_and_tender rejectedLpoBid, int LPOId);
        RequisitionNoteRegRespObj SaveSingleFile(IFormFile file);
    }
}
