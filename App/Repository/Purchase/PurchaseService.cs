...
namespace Puchase_and_payables.Repository.Purchase
{
    public class PurchaseService : IPurchaseService
    {
        ...
        async Task<bool> IPurchaseService.ApproveCurrentBidForThisLPO(cor_bid_and_tender currentBid, purch_plpo _ThisBidLPO)
        {
            ...
            await AddUpdateBidAndTender(currentBid);
            return true;
        }

        async Task<bool> IPurchaseService.LinkThisProposalsToCurrentLPOAsync(cor_bid_and_tender currentBid)
        {
            var terms = _dataContext.cor_paymentterms.Where(q => q.BidAndTenderId == currentBid.BidAndTenderId).ToList();
            foreach (var term in terms)
            {
                term.LPOId = currentBid.PLPOId;
                term.CompanyId = currentBid.CompanyId;
                await AddUpdatePaymentTermsAsync(term);
            }
            return true;
        }

        async Task<bool> IPurchaseService.DisapproveOtherLostbids(cor_bid_and_tender currentBid)
        {
            var otherBids = _dataContext.cor_bid_and_tender.Where(q => q.LPOnumber == currentBid.LPOnumber && q.BidAndTenderId != currentBid.BidAndTenderId).ToList();
            if (otherBids.Count() > 0)
            {
                foreach (var otherbid in otherBids)
                {
                    ...
                    await AddUpdateBidAndTender(otherbid);
                }
            }
            return true;
        }
        async Task<bool> IPurchaseService.ReactivateOtherBids(cor_bid_and_tender rejectedLpoBid, int LPOId)
        {
            var lostBids =  _dataContext.cor_bid_and_tender.Where(q => q.PLPOId == LPOId);
            if (lostBids.Count() > 0)
            {
                foreach (var otherbid in lostBids.ToList())
                {
                    var item = await GetBidAndTender(otherbid.BidAndTenderId);
                    if (item != null)
                    { 
                        ...
                        {
                            item.ApprovalStatusId = (int)ApprovalStatus.Awaiting;
                            item.DecisionResult = (int)DecisionResult.Non_Applicable;
                            item.Paymentterms = new List<cor_paymentterms>();
                        }
                        else if(item.BidAndTenderId == rejectedLpoBid.BidAndTenderId)
                        {
                            item.ApprovalStatusId = (int)ApprovalStatus.Awaiting;
                            item.DecisionResult = (int)DecisionResult.Non_Applicable;  
                        }
                        await AddUpdateBidAndTender(item);
                    }
                }
            }
            return true;
        }
        async Task<bool> IPurchaseService.RejectThisBid(cor_bid_and_tender rejectedLpoBid)
        {
            if (rejectedLpoBid.Paymentterms.Count() > 0)
            {
                foreach (var term in rejectedLpoBid.Paymentterms)
                {
                    var item =  _dataContext.cor_paymentterms.Find(term.PaymentTermId);
                    if (item != null)
                    {
                        item.LPOId = 0;
            ...
            await AddUpdateBidAndTender(rejectedLpoBid);
            return true;
        }
        ...
        public async Task<bool> ShareTaxToPhasesIthereIsAsync(purch_plpo currentLpo)
        {
            ...
            var selected = currentLpo.Taxes.Split(',').ToList();
            if (!string.IsNullOrEmpty(currentLpo.Taxes) && selected.Any())
            {
                var selectedint = currentLpo.Taxes.Split(',').Select(int.Parse).ToList();
                var taxes = _dataContext.cor_taxsetup.Where(e => selectedint.Contains(e.TaxSetupId)).ToList();
                if (paymentPhases.Count() > 0)
                {
                    foreach (var phase in paymentPhases)
                    {
                        phase.CompanyId = currentLpo.CompanyId;
                        if (phase.Amount != 0)
                        {
        ...
        public purch_plpo BuildLPODomianObject(purch_prndetails model, purch_requisitionnote prn)
        {
            return new purch_plpo
            {
                Active = true,
                DeliveryDate = prn?.ExpectedDeliveryDate?? DateTime.UtcNow,
                Address = prn.DeliveryLocation,
        ...
        {
            var lostbids = await _dataContext.cor_bid_and_tender.Where(a => a.PLPOId == lpo.PLPOId && a.BidAndTenderId != lpo.WinnerSupplierId).ToListAsync();
            if (lostbids.Count() > 0)
            {
        ...
        public async Task SendEmailToSuppliersAsync(EmailMessageObj em, string description)
        {
            ...
            foreach (var supplier in em.ToAddresses)
            {
                var user = await _userManager.FindByEmailAsync(supplier.Address);
                if (user != null)
                {
                    ids.Add(user.Id);
                }
                if (ids.Count() > 0) em.UserIds = string.Join(',', ids);
            }
            em.Module = (int)Modules.PURCHASE_AND_PAYABLES;
        ...
        RequisitionNoteRegRespObj IPurchaseService.SaveSingleFile(IFormFile file)
        {
            var response = new RequisitionNoteRegRespObj();
            if (file != null)
            { 
                if (file.Length > 0)
                {
            ...
            response.Status.Message.FriendlyMessage = "Evidence Not Found";
            return response;
        }
    }
}
