...
namespace GODP.APIsContinuation.Repository.Inplimentation
{
    public class SupplierRepository : ISupplierRepository
    {
        ...
        bool ISupplierRepository.IsProfileCompleted(int supplierId)
        {
            var hasTopSuppliers = _dataContext.cor_topsupplier.Any(e => e.SupplierId == supplierId);
            var hasTopClients = _dataContext.cor_topclient.Any(e => e.SupplierId == supplierId);
            var hasTopDocuments = _dataContext.cor_supplierdocument.Any(e => e.SupplierId == supplierId);
            var hasTopBusinessOwners = _dataContext.cor_supplierbusinessowner.Any(e => e.SupplierId == supplierId);
            var hasTopAuthorizations = _dataContext.cor_supplierauthorization.Any(e => e.SupplierId == supplierId);
            if (hasTopSuppliers && hasTopClients && hasTopDocuments && hasTopBusinessOwners && hasTopAuthorizations)
                return true;
            return false;
        }
