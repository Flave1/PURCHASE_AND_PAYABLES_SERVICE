...
namespace Puchase_and_payables.Requests
{
    public class FinanceServerRequest : IFinanceServerRequest
    {
        ...
        async Task<OtherBanksResp> IFinanceServerRequest.GetOtherBanksAsync()
        {
            try
            {
                var gosGatewayClient = _httpClientFactory.CreateClient("GOSDEFAULTGATEWAY");
                ...
                var response = new OtherBanksResp();


                var result = await gosGatewayClient.GetAsync(ApiRoutes.FinanceServerRequest.GET_OTHER_BANKS);
                var resultString = await result.Content.ReadAsStringAsync();
                response = JsonConvert.DeserializeObject<OtherBanksResp>(resultString);
                if (!result.IsSuccessStatusCode)
                {
        ...
        async Task<OtherBanksResp> IFinanceServerRequest.GetSingleOtherBanksAsync(string code)
        {
            try
            {
                var gosGatewayClient = _httpClientFactory.CreateClient("GOSDEFAULTGATEWAY");
                ...
                var result = await gosGatewayClient.GetAsync($"{ApiRoutes.FinanceServerRequest.GET_SINGLE_OTHER_BANK}?OtherBankCode={code}");
                var resultString = await result.Content.ReadAsStringAsync();
                response = JsonConvert.DeserializeObject<OtherBanksResp>(resultString);
                if (!result.IsSuccessStatusCode)
                {
