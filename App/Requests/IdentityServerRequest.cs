    ...
    public class IdentityServerRequest : IIdentityServerRequest
    {
        ...
        public async Task<HttpResponseMessage> GotForApprovalAsync(GoForApprovalRequest request)
        {
            ...
            return await _retryPolicy.ExecuteAsync(async () =>
            {
                try
                {
                    ...
                    _logger.Information(resultObj.Status.Message.FriendlyMessage);
                    if (!result.IsSuccessStatusCode)
                    {

                        _logger.Information(resultObj.Status.Message.TechnicalMessage);
                        _logger.Information($"{resultObj.HasWorkflowAccess}-{resultObj.EnableWorkflow}-{resultObj.ApprovalProcessStarted}");
                        new StaffApprovalRegRespObj
                        {
                            ResponseId = (int)result.StatusCode,
                            Status = new APIResponseStatus
                            {
        ...
        public async Task<CommonLookupRespObj> GetAllDocumentsAsync()
        {
            ...
            try
            {
                ...
                var resultString = await result.Content.ReadAsStringAsync();
                response = JsonConvert.DeserializeObject<CommonLookupRespObj>(resultString);
                if (!result.IsSuccessStatusCode)
                {
                    response.Status.IsSuccessful = false;
                    response.Status.Message.FriendlyMessage = $"{result.ReasonPhrase}  {(int)result.StatusCode}  {result.Content}";
        ...
        public async Task<UserRoleRespObj> GetUserRolesAsync()
        {
            try
            {
                ...
                _logger.Information(result.RequestMessage.RequestUri.ToString());
                var resultString = await result.Content.ReadAsStringAsync();
                response = JsonConvert.DeserializeObject<UserRoleRespObj>(resultString);
                _logger.Information($"{response.Status.IsSuccessful.ToString()} is another status");
                //if (!result.IsSuccessStatusCode)
                //{
                //    response.Status.IsSuccessful = false;
                //    response.Status.Message.FriendlyMessage = $"{result.ReasonPhrase}  {(int)result.StatusCode}  {result.Content}";
                //    //sthrow new Exception($"{response}");
                //} 
                return response;
            }
            catch (Exception ex)
        ...
        public async Task<HttpResponseMessage> CheckTrackedAsync(string token, string userid)
        {
            try
            {
                ...
                string authorization = _accessor.HttpContext.Request.Headers["Authorization"];
                gosGatewayClient.DefaultRequestHeaders.Add("Authorization", authorization); 
                return await gosGatewayClient.PostAsync(ApiRoutes.IdentitySeverRequests.TRACKED, byteContent); 
            }
            catch (Exception ex)
            {
                #region Log error 
